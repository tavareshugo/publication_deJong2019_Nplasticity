...
# Another panel containing the comparison with GWAS-type approach (using ~1.4M SNPs)
p5 <- qtl_results %>% filter(trait == "totalbr_mean_D") %>% 
  ggplot(aes(bp/1e6, -log10(p))) +
  geom_point(data = temp, colour = "grey") +
  geom_line() +
  facet_grid( ~ chromosome, scales = "free", space = "free_x") +
  labs(x = "Mb") +
  geom_hline(yintercept = 3.5, linetype = "dashed") +
