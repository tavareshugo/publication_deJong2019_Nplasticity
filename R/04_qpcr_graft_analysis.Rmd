---
title: Shoot branching plasticity depends on shoot genotype
...
Then calculate the average across three technical replicates for each sample.
...
Now we calculate the average for the two reference genes:
...
Now we calculate the first delta-Cp: $\Delta Cp = Cp_{target} - Cp_{reference}$
...
Now we calculate average Cp of each gene in the control condition:
...
And finally we calculate the delta-delta-Cp:
...
And here is the figures presented in the paper, which summarise things as 
means +/- CI:
...
We thus use a random term to try and account for intra-class correlation of 
...
slopes for nitrate. 
Although we're not interested in it, experiment is added as a fixed effect since 
...
This essentially agrees with ANOVA:

```{r}
aov_full <- aov(totalbr ~ nitrate*shoot_type + nitrate*root_type + 
               experiment + nitrate*id, 
             data = graft)
summary(aov_full)
```
