---
title: Arabidopsis shows extensive natural variation in shoot branching response to
  nitrate supply
...
The code in this document reproduces figures 1-4 and S1 of the paper. It requires 
the packages `tidyverse` and `patchwork`.
...
The following code puts these plots together:
...
This figure shows the difference between number of shoot branches at the 2-silique 
...
This shows the correlation between shoot branches and silique number at the 
senescence stage:
...
observed in the "random slopes" model produced for Figure 2.

It also shows the correlation between plasticity and flowering time.

Here, I produce each plot at a time.
...
And now all the plots can be put together:
...
```{r figure4A}
# Plot of total branches against plasticity. 
# I add Pearson's correlation to the plot
...
Relationship between plasticity and average fruit yield:

```{r}
acc_plas_sen %>% 
  mutate(mean_sil = (totalsil_mean_hn + totalsil_mean_ln)/2) %>% 
  # mutate(totalbr_mean_D = as.vector(scale(totalbr_mean_D)),
  #        mean_sil = as.vector(scale((totalsil_mean_hn + totalsil_mean_ln)/2))) %>% 
  # #filter(set == "senescence") %>% head
...
```{r}
temp <- acc_sum_sen %>% 
  group_by(id_gwapp, name) %>% 
  summarise(totalbr_mean_D = totalbr_mean[nitrate == "HN"] - totalbr_mean[nitrate == "LN"],
            totalsil_mean_all = (totalsil_mean[nitrate == "HN"] * totalsil_n[nitrate == "HN"] + totalsil_mean[nitrate == "LN"] * totalsil_n[nitrate == "LN"])/(totalsil_n[nitrate == "HN"] + totalsil_n[nitrate == "LN"]))

fit <- lm(totalsil_mean_all ~ totalbr_mean_D, data = temp)

ggplot(temp, aes(totalbr_mean_D, totalsil_mean_all)) +
  geom_point() +
  geom_abline(intercept = fit$coefficients[1], slope = fit$coefficients[2], colour = "steelblue", size = 1) +
  geom_hline(aes(yintercept = mean(totalsil_mean_all)), colour = "grey48", linetype = "dashed") +
  geom_vline(aes(xintercept = mean(totalbr_mean_D)), colour = "grey48", linetype = "dashed")
```
