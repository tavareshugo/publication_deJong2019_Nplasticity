...
This document was produced using the statistical package _R_, and can be used to 
reproduce our analysis based on the code show.
...
For each there are three data files in the `./data/phenotypes` directory. The 
files have the following sufixes:

* `_individual` contains phenotype data for each individual plant.
* `_summarised` contains phenotype data summarised for each line (for example 
mean trait value per line).
* `_plasticity` is similar to the one above, but in a different format and with 
plasticities (difference between HN and LN) calculated.
...
* is there significant _variation_ in that plasticity between lines?

Our definition of "plasticity" is simply the difference between the mean trait 
values on `HN - LN`. Therefore, for modeling purposes, I set the variable 
`nitrate` as a factor, with LN as the reference value:
...
on height, is quite detailed as it sets the stage for the analysis carried out 
throughout the rest of the document. The other sections are less detailed.
...
for the trait being considered (this might strictly speaking not be true - 
...
The discrepancy is likely due to the slight heteroskedasticity in our residuals.

An now we can examine a summary of this model:
...
We can see that this is roughly true.
...
The result suggests that we should reject the null hypothesis that the two models 
explain a similar amount of variance in height, leading us to conclude that there 
is significant plasticity for height in response to nitrate.
...
The variance estimates from the model are:
...
to plasticity in this model, the trait is well correlated between environments, 
with an estimated correlation of 
`r cor(magic_plas$height_mean_hn, magic_plas$height_mean_ln)` from the 
data.
...
This gives some problems using the model assuming gaussian errors:
...
In the model, this substantially attenuates the heteroskedasticity, although the 
residuals still depart from normality at the tails:
...
components is more challenging in that case. We will show how that approach gives 
qualitatively similar results at the end of this section.
...
The estimate is actually not matching that of the data. This is because these 
...
Confirming very little variation due nitrate and so virtually no plasticity.
...
distribution (except towards the tails) and their variance does not seem to 
increase with the fitted values.
...
The same procedure as above is used for the accessions. I will explain individual 
steps more sparingly.
...
These are not significantly different using p < 0.01! 
Suggests there is no significant plasticity for the trait. Given this, it's 
hardly worth continuing, but we do so to extract variance components.
...
```{r}
ggplot(acc_ind_sen, aes(sqrt(totalsil))) + 
  geom_histogram() +
  facet_grid(~ nitrate)
```
...
with the two traits being correlated (shown in Figure X of paper).
...
This will be done to produce the variance-component barplots:
