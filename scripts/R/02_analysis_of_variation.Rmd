...
pdf("./figures/figure1_new.pdf", width = 4.5, height = 2.5)
all_trait_vars %>% 
...
```{r}
pdf("./figures/figure1_new_supplement.pdf", width = 4.5, height = 2.5)
all_trait_vars %>% 
  mutate(trait = factor(trait, levels = c("bolt", "height", "branches", "branches_sen", "sil_sen")),
         set = factor(set, levels = c("MAGIC lines", "Accessions"))) %>% 
  group_by(set, trait) %>% 
...
```{r}
# early flowering magic lines
magic_filter <- magic_plas %>% filter(bolt_mean_ln < 25) %>% pull(id_kover)

# Ordered by average flowering time in both nitrates
p1 <- magic_sum %>% 
  filter(id_kover %in% magic_filter) %>% 
  group_by(id_kover) %>% 
  mutate(bolt_avg = mean(bolt_mean)) %>% 
  ungroup() %>% 
  mutate(id_kover = fct_reorder(id_kover, -bolt_avg)) %>% 
  ggplot(aes(nitrate, totalbr_mean)) + 
  geom_line(aes(group = id_kover, colour = bolt_avg), alpha = 0.5, size = 1) +
...
temp <- magic_extremes %>% 
  select(id_kover, plastic_category) %>% 
  full_join(magic_sum, by = "id_kover") %>% 
  filter(id_kover %in% magic_filter)

temp %>% 
  ggplot(aes(nitrate, totalbr_mean, group = id_kover)) + 
  geom_line(alpha = 0.3) +
  geom_line(data = filter(temp, plastic_category == "Most plastic"), aes(colour = "25 most plastic")) +
  geom_line(data = filter(temp, plastic_category == "Least plastic"), aes(colour = "25 least plastic")) +
  geom_point(data = filter(temp, plastic_category == "Most plastic"), aes(colour = "25 most plastic")) +
  geom_point(data = filter(temp, plastic_category == "Least plastic"), aes(colour = "25 least plastic")) +
  scale_colour_viridis_d()

p2 <- magic_extremes %>% 
  ggplot(aes(nitrate, totalbr_mean, group = id_kover, colour = plastic_category)) + 
  geom_line() +
  geom_point() +
  scale_colour_viridis_d() + theme(legend.position = "none", text = element_text(size = 12))
...
Distribution of plasticity in MAGIC and Accessions

```{r}
select(acc_plas, id = id_leyser, totalbr_mean_D) %>% 
  mutate(set = "accessions") %>% 
  bind_rows(select(magic_plas, id = id_leyser, totalbr_mean_D)) %>% 
  mutate(set = ifelse(is.na(set), "MAGIC", set)) %>% 
  ggplot(aes(set, totalbr_mean_D)) + 
  geom_hline(yintercept = 0, linetype = 2, colour = "grey") +
  geom_violin(fill = "grey", alpha = 0.5) +
  geom_boxplot(width = 0.05) +
  labs(x = "", y = "Shoot branching plasticity\n(HN - LN)") +
  theme(text = element_text(size = 12))
```

#### PCA

Trying out principal components analysis on these data.
...
pca_var <- tidy(magic_pca, "d") %>% 
  mutate(label = paste0("PC", PC, " (", round(percent*100), "%)"))

p1 <- tidy(magic_pca, "pcs") %>% 
  ggplot(aes(PC, cumulative*100)) + geom_point() + geom_line() +
  labs(x = "PC", y = "Cumulative variance explained (%)")

p2 <- augment(magic_pca, magic_sum)  %>% 
  ggplot(aes(.fittedPC1, .fittedPC2)) + 
  geom_point(aes(colour = nitrate)) +
  scale_colour_viridis_d() +
  labs(x = pca_var$label[1], y = pca_var$label[2])

p3 <- magic_pca$rotation %>% 
  as_tibble(rownames = "trait") %>% 
  mutate(trait = str_remove(trait, "_mean")) %>% 
  ggplot() +
  geom_segment(aes(x = 0, xend = PC1, y = 0, yend = PC2), arrow = arrow(length = unit(0.1, "in"), type = "closed"), colour = "steelblue") +
  geom_text(aes(x = PC1, y = PC2, label = trait), nudge_y = 0.03) +
  labs(x = pca_var$label[1], y = pca_var$label[2])

#pdf("~/temp/pca_magic.pdf", width = 8, height = 4)
p2 + theme(text = element_text(size = 20)) + p3 + theme(text = element_text(size = 20)) + plot_layout(ncol = 1)
#dev.off()
```
...
acc_pca <- acc_sum %>% 
  filter(set == "silique") %>% 
  select(matches("_mean$"), -totalsil_mean, -boltLog_mean, -totalbrSqrt_mean) %>% 
  mutate(inactive_mean = nodes_mean - cauline_mean) %>% 
  prcomp(center = TRUE, scale = TRUE)

pca_var <- tidy(acc_pca, "d") %>% 
  mutate(label = paste0("PC", PC, " (", round(percent*100), "%)"))
...
magic_plas %>% 
  select(matches("_mean_hn|_mean_ln")) %>% 
  select(matches("totalbr|height|bolt")) %>% 
  rename_all(funs(str_remove(., "_mean"))) %>% 
  correlate(method = "spearman") %>% 
  network_plot(min_cor = 0.2, colours = c("steelblue", "white", "brown")) +
  ggtitle("MAGIC")

acc_plas %>% 
  filter(set == "silique") %>% 
  select(matches("_mean_hn|_mean_ln"), -matches("Log|Sqrt")) %>% 
  select(matches("totalbr|height|bolt")) %>% 
  rename_all(funs(str_remove(., "_mean"))) %>% 
  correlate(method = "spearman") %>% 
  network_plot(min_cor = 0.2, colours = c("steelblue", "white", "brown")) +
...
Using [umap](https://cran.r-project.org/web/packages/umap/index.html) package.

```{r}
library(umap)
early_magic <- magic_plas %>% filter(bolt_mean_ln < 25) %>% pull(id_leyser)

magic_umap <- magic_sum %>% 
  #filter(id_leyser %in% early_magic) %>% 
  select(matches("_mean$")) %>% 
  umap()
...
magic_umap$layout %>% 
  as_tibble() %>% 
  bind_cols(magic_sum) %>% 
  ggplot(aes(V1, V2)) + geom_point(aes(colour = nitrate))
...
magic_umap$layout %>% 
  ...
  bind_cols(magic_sum) %>% 
  group_by(id_leyser) %>% 
  mutate(plast = totalbr_mean[nitrate == "HN"] - totalbr_mean[nitrate == "LN"]) %>% 
  ungroup() %>% 
  ggplot(aes(V1, V2, colour = plast)) + geom_point() +
...
magic_umap$layout %>% 
  ...
  mutate(plast = totalbr_mean[nitrate == "HN"] - totalbr_mean[nitrate == "LN"]) %>% 
  ungroup() %>% 
  ggplot(aes(V1, plast, colour = nitrate)) + geom_point()
```
