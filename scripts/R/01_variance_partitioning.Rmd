...
```{r}
magic_ind %>% 
  ggplot(aes(height, colour = nitrate)) +
  stat_ecdf() + ggtitle("ecdf")

magic_ind %>% 
  ggplot(aes(sample = scale(height), colour = nitrate)) +
  stat_qq() + geom_abline() + ggtitle("QQ plot")
```
...
explains ~40% of the phenotypic variance. 

We can also test if this "plasticity model" explains a significant amount of 
variance compared to the previous "genetic model":

```{r}
anova(m0_magic_height, m1_magic_height) %>% tidy
```

The result suggests that the likelihood difference between the two models 
is very unlikely assuming they both explain a similar amount of variance in height.

However, it's worth noting that the interpretation of p-values should be done with 
care. Our sample sizes are quite big, so even small effects can increase the likelihood 
of the model enough and thus lead to very small p-value (this will be obvious below 
on flowering time). Although we report the results for these likelihood ratio 
tests here, they're actually rather uninformative for our purpose, which is 
to explore how the variance in phenotype can be partitioned into genetic and 
...
```{r}
# Testing changing contrast so that we get the overall average as the intercept
temp <- lmer(height ~ nitrate + (nitrate|id_leyser), data = magic_ind, contrasts = list(nitrate = contr.helmert(2)))
summary(temp)
...
```{r}
magic_ind %>% 
  ggplot(aes(bolt, colour = nitrate)) +
  stat_ecdf() + ggtitle("ecdf")

magic_ind %>% 
  ggplot(aes(sample = scale(bolt), colour = nitrate)) +
  stat_qq() + geom_abline() + ggtitle("QQ plot")
```
...
# plot
magic_ind %>% 
  ggplot(aes(boltLog, colour = nitrate)) +
  stat_ecdf() + ggtitle("ecdf")

magic_ind %>% 
  ggplot(aes(sample = scale(boltLog), colour = nitrate)) +
  stat_qq() + geom_abline() + ggtitle("QQ plot")
```
...
Although the p-value of this test is small, the effect size from the model 
is very small (an average of 1 day of flowering difference seems biologically 
insignificant).
...
Again, despite the very low p-value of the test, from the summary above, we can 
see that the variance attributed to genetic plasticity is actually quite low. 
...
mean values on HN or LN and the plasticity. In other words, late flowering 
...
The two traits are highly correlated in the two nitrate treatments, which 
biologically suggests low variation in plasticity for this trait.
...
[ but by now we know that we don't like p-values :) ]:
...
genotypic variance on LN. We don't need the test, but here it is:
...
# MAGIC line models
m2_magic_height <- lmer(height ~ nitrate + (nitrate|id_leyser), data = magic_ind, contrasts = list(nitrate = contr.helmert(2)))
m2_magic_bolt <- lmer(boltLog ~ nitrate + (nitrate|id_leyser), data = magic_ind, contrasts = list(nitrate = contr.helmert(2)))
m2_magic_totalbr <- lmer(totalbr ~ nitrate + (nitrate|id_leyser), data = magic_ind, contrasts = list(nitrate = contr.helmert(2)))

# Accession 2-silique models
m2_acc_height <- lmer(height ~ nitrate + (nitrate|id_leyser), data = acc_ind, contrasts = list(nitrate = contr.helmert(2)))
m2_acc_bolt <- lmer(boltLog ~ nitrate + (nitrate|id_leyser), data = acc_ind, contrasts = list(nitrate = contr.helmert(2)), 
                    control = lmerControl(optimizer = "Nelder_Mead"))
m2_acc_totalbr <- lmer(totalbr ~ nitrate + (nitrate|id_leyser), data = acc_ind, contrasts = list(nitrate = contr.helmert(2)))

# Accession senescence models
m2_accsen_totalbr <- lmer(totalbr ~ nitrate + (nitrate|id_leyser), data = acc_ind_sen, contrasts = list(nitrate = contr.helmert(2)))
m2_accsen_totalsil <- lmer(totalsilSqrt ~ nitrate + (nitrate|id_leyser), data = acc_ind_sen, contrasts = list(nitrate = contr.helmert(2)))
```
...
This table shows the genetic correlations across environments (nitrate), which is 
used to annotated the plot:
...
all_trait_vars %>% 
  ...
  scale_fill_manual(values = c("#999999", "#56B4E9", "#009E73", "#E69F00", "#F0E442"),
                    labels = c("Residual", "Genotype x Nitrate", "Nitrate", "Genotype (HN)", "Genotype (LN)")) +
  #scale_fill_manual(values = c("white", "gray48", "grey", "black")) +
  #geom_text(size = 3, position = position_stack(vjust = 0.5)) +
  labs(y = "% variance", x = "Trait") +
  geom_text(data = corr_labels, aes(x = trait, y = 102, label = cor_ln_hn), inherit.aes = FALSE,
            size = 3)
```
...
Make plot of genetic coefficient of variation:
...
all_trait_vars %>% 
  ...
  labs(y = "Coefficient of variation", x = "Trait")
```
...
I'm trying to understand how to fit the model similar to the ones above, but 
using `MCMCglmm`.

```{r}
library(MCMCglmm)

# This was the model fitted with lme4
# lmer(height ~ nitrate + (nitrate|id_leyser), data = acc_ind)

# Trying to do this with MCMCglmm package
mcmc_branches <- MCMCglmm(totalbr ~ nitrate, random = ~ us(nitrate):id_leyser, rcov = ~ units, 
                          nitt = 23000, burnin = 3000, thin = 20, 
                          data = as.data.frame(acc_ind))

mcmc_height <- MCMCglmm(height ~ nitrate, random = ~ us(nitrate):id_leyser, rcov = ~ units, 
                        nitt = 23000, burnin = 3000, thin = 20, 
                        data = as.data.frame(acc_ind))


# Needed to change the priors, otherwise the model failed. I'm not sure why, 
## but suspect it's because the correlation is close to 1 between two nitrate environments...
## Got the clue from this forum: https://stat.ethz.ch/pipermail/r-sig-mixed-models/2013q4/021286.html
prior_bolt <- list(G = list(G1 = list(V = diag(2), nu = 0.002, alpha.mu=c(0,0), alpha.V=diag(2)*1000)))
mcmc_bolt <- MCMCglmm(boltLog ~ nitrate, random = ~ us(nitrate):id_leyser, rcov = ~ units, 
                 nitt = 23000, burnin = 3000, thin = 20, prior = prior_bolt, 
                 data = as.data.frame(acc_ind))
```

```{r}
summary(mcmc_branches)
```
...
```{r}
diag(autocorr(temp$VCV)[2, , ])
cor(temp$VCV)
colMeans(temp$VCV)
HPDinterval(temp$VCV/rowSums(temp$VCV))
```

```{r}
vars_lmm <- extractVarsLmm(m2_acc_totalbr)
vars_lmm
```

```{r}
vars_lmm$cor_ln_hn*(sqrt(vars_lmm$var_ln) * sqrt(vars_lmm$var_hn))
```
...
```{r}
summary(m2_acc_totalbr)
```



```{r}
m2_acc_bolt <- lmer(boltLog ~ nitrate + (nitrate|id_leyser), data = acc_ind %>% filter(!is.na(bolt)), 
                    control = lmerControl(optimizer ="Nelder_Mead"))
summary(m2_acc_bolt)
```
...
Now try to fit a multi-trait model. Just on HN to start with.

```{r}
acc_hn <- acc_ind %>% 
  filter(nitrate == "HN" & !is.na(height) & !is.na(totalbr)) %>% 
  mutate_at(vars(totalbr, height), funs(scale))

acc_hn %>% 
  ggplot(aes(totalbr, height)) + geom_point()

cor(scale(acc_hn$totalbr), scale(acc_hn$height))
```
...
```{r}
multi_mcmc <- acc_hn %>% 
  select(id_leyser, totalbr, height) %>% 
  gather("phene", "value", totalbr, height) %>% 
  as.data.frame() %>% 
  MCMCglmm(value ~ phene, random = ~ us(phene):id_leyser, data = .,
           nitt = 23000, burnin = 3000, thin = 20)
```
...
```{r}
0.3053493/(sqrt(0.4645477) * sqrt(0.3360943))
```
...
multi_lmer <- acc_hn %>% 
  select(id_leyser, totalbr, height) %>% 
  gather("trait", "value", totalbr, height) %>% 
  group_by(trait) %>% 
  mutate(scaled = scale(value)) %>% 
  lmer(scaled ~ trait + (trait|id_leyser), data = .)
summary(multi_lmer)
extractVarsLmm(multi_lmer)
```
...
multi_mcmc2 <- acc_hn %>% 
  select(id_leyser, totalbr, height) %>% 
  as.data.frame() %>% 
  MCMCglmm(cbind(totalbr, height) ~ 1, random = ~ us(trait):id_leyser, data = .,
